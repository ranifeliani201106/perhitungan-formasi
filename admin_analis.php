...
// Query untuk mengambil data dari tabel verifikasi_negosiator
$queryVerifikasi = "SELECT * FROM verifikasi_analis";
$stmtVerifikasi = $pdo->prepare($queryVerifikasi);
$stmtVerifikasi->execute();
$verifikasi = $stmtVerifikasi->fetchAll(PDO::FETCH_ASSOC);

// Hitung total data untuk pagination
$totalQuery = "SELECT COUNT(*) FROM hasil_analis";
if ($id_user || $filter_ruang_lingkup) {
    $totalQuery .= " WHERE";
    if ($id_user) {
...
<div class="container mt-5">
    <h2 class="text-center mb-4">Hasil Perhitungan Analis Perdagangan</h2>

    <?php if ($id_user && $userData): ?>
        <h4 class="text-white text-center">Hasil untuk: <?php echo htmlspecialchars($userData['namapic']); ?> - <?php echo htmlspecialchars($userData['namainstansi']); ?></h4>
    <?php endif; ?>
                ...
                <?php if (count($hasil) > 0): ?>
                    <?php foreach ($hasil as $index => $row): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row['pertama']); ?></td>
                            <td><?php echo htmlspecialchars($row['muda']); ?></td>
                            <td><?php echo htmlspecialchars($row['madya']); ?></td>
                            <td><?php echo htmlspecialchars($row['utama']); ?></td>
                            <td><?php echo htmlspecialchars($row['keterangan']); ?></td>
                            <td class="d-flex">
                                <a href="edit_analis.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning me-2">Edit</a>
                            </td>
            ...
            <?php if (count($verifikasi) > 0): ?>
                <?php foreach ($verifikasi as $index => $row): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['keterangan']); ?></td>
                        <td class="d-flex">
                                <a href="hapus_analis.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus data ini?');">Hapus</a>
                            </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                <tr>
                    <td colspan="9" class="text-center">Tidak ada data verifikasi</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
...
<footer class="mt-auto bg-dark text-white text-center py-3">
    <p class="mb-0">© 2024 Admin Panel - Hasil Analis</p>
</footer>
