...
// Query untuk mengambil data dari tabel verifikasi_pengamat
$queryVerifikasi = "SELECT * FROM verifikasi_pengamat";
$stmtVerifikasi = $pdo->prepare($queryVerifikasi);
$stmtVerifikasi->execute();
$verifikasi = $stmtVerifikasi->fetchAll(PDO::FETCH_ASSOC);

// Hitung total data untuk pagination
$totalQuery = "SELECT COUNT(*) FROM hasil_pengamat";
if ($id_user || $filter_ruang_lingkup) {
    $totalQuery .= " WHERE";
    if ($id_user) {
...
<div class="container mt-5">
    <h2 class="text-center mb-4">Hasil Perhitungan Pengamat</h2>
    ...
    <?php if (count($hasil) > 0): ?>
        <?php foreach ($hasil as $index => $row): ?>
            <tr>
                ...
                <td><?php echo number_format((float)$row['pemula'], 2); ?></td>
                ...
                <td class="d-flex">
                    <a href="edit_pengamat.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning me-2">Edit</a>
                </td>
            </tr>
        <?php endforeach; ?>
    <?php else: ?>
    ...
    <?php if (count($verifikasi) > 0): ?>
        <?php foreach ($verifikasi as $index => $row): ?>
            <tr>
                ...
                <td><?php echo htmlspecialchars($row['keterangan']); ?></td>
                <td class="d-flex">
                    <a href="hapus_pengamat.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Yakin ingin menghapus data ini?');">Hapus</a>
                </td>
            </tr>
        <?php endforeach; ?>
    <?php else: ?>
...
<footer class="mt-auto bg-dark text-white text-center py-3">
    <p class="mb-0">© 2024 Admin Panel - Hasil Pengamat Tera</p>
</footer>
