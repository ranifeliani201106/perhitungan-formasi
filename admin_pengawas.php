...
// Query untuk mengambil data dari tabel verifikasi_pengawas
$queryVerifikasi = "SELECT * FROM verifikasi_pengawas";
$stmtVerifikasi = $pdo->prepare($queryVerifikasi);
$stmtVerifikasi->execute();
$verifikasi = $stmtVerifikasi->fetchAll(PDO::FETCH_ASSOC);

// Hitung total data untuk pagination
$totalQuery = "SELECT COUNT(*) FROM hasil_pengawas";
if ($id_user || $filter_ruang_lingkup) {
    $totalQuery .= " WHERE";
    if ($id_user) {
...
<div class="container mt-5">
    <h2 class="text-center mb-4">Hasil Perhitungan Pengawas Perdagangan</h2>
                ...
                <?php if (count($hasil) > 0): ?>
                    <?php foreach ($hasil as $index => $row): ?>
                        <tr>
                            ...
                            <td class="d-flex">
                                <a href="edit_pengawas.php?id=<?php echo $row['id']; ?>" class="btn btn-sm btn-warning me-2">Edit</a>
                            </td>
            ...
            <?php if (count($verifikasi) > 0): ?>
                <?php foreach ($verifikasi as $index => $row): ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($row['keterangan']); ?></td>
                        <td class="d-flex">
                                <a href="hapus_pengawas.php?id=<?php echo $row['id']; ?>"
...
<footer class="mt-auto bg-dark text-white text-center py-3">
    <p class="mb-0">© 2024 Admin Panel - Hasil Pengawas Perdagangan</p>
</footer>
