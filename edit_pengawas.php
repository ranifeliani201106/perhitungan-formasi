...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header('Location: hasil_pengawas.php'); // Redirect jika tidak ada id
    exit;
}
...
$query = "SELECT * FROM hasil_pengawas WHERE id = :id";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($ruang_lingkup) {
        case "Pengawasan Kegiatan Perdagangan":
            $skr = 66.00;
            $persentase = [0.42, 0.36, 0.19, 0.03];
            break;
        case "pengawasan Barang Beredar dan/atau Jasa":
            $skr = 175.00;
            $persentase = [0.40, 0.37, 0.20, 0.03];
            break;
        case "pengawasan Alat Ukur, Alat Takar, Alat Timbang, dan Alat Perlengkapan":
            $skr = 2.00;
            $persentase = [0.41, 0.39, 0.15, 0.05];
            break;
        case "pengawasan BDKT":
            $skr = 1.00;
            $persentase = [0.42, 0.38, 0.17, 0.03];
            break;
        case "pengawasan penggunaan satuan ukuran":
            $skr = 3.00;
            $persentase = [0.45, 0.33, 0.18, 0.04];
            break;
        case "Pengawasan Perdagangan Berjangka Komoditi, Sistem Resi Gudang dan Pasar Lelang Komodita":
            $skr = 170.00;
            $persentase = [0.38, 0.32, 0.26, 0.04];
            break;
        ...
        case "Penyidikan":
            $skr = 880.00;
            $persentase = [0.20, 0.35, 0.41, 0.04];
            break;
        case "Penindakan":
            $skr = 66.00;
            $persentase = [0.20, 0.35, 0.40, 0.05];
            break;
        default:
    ...
    $insertQuery = "INSERT INTO verifikasi_pengawas
...
$insertStmt->bindParam(':id_asal', $id, PDO::PARAM_INT);
    ...
    if ($insertStmt->execute()) {
        header('Location: admin_pengawas.php?success=edit'); // Redirect ke halaman utama
        exit;
    } else {
...
<head>
    ...
    <title>Edit Hasil Pengawas Perdagangan</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body>
<div class="container mt-5">
    <h2>Edit Data Hasil Pengawas Perdagangan</h2>
    <form method="POST" action="">
        ...
        <a href="admin_pengawas.php" class="btn btn-secondary">Batal</a>
    </form>
</div>
